...
if ( ! empty( $theme_options['theme_select_uitgelicht'] ) ) {
	...
	$entry = get_post( $theme_options['theme_select_uitgelicht'] );
	...
	<tr>
		<th>theme_xx2_vrije_invoer_text</th>
		<td>
			<?php $controls->textarea( 'theme_xx2_vrije_invoer_text' ); ?>
...
$theme_xx2_vrije_invoer_title = isset( $theme_options['theme_xx2_vrije_invoer_title'] ) ? $theme_options['theme_xx2_vrije_invoer_title'] : '';
$theme_xx2_vrije_invoer_text  = isset( $theme_options['theme_xx2_vrije_invoer_text'] ) ? $theme_options['theme_xx2_vrije_invoer_text'] : '';
$theme_xx2_vrije_invoer_image = isset( $theme_options['theme_xx2_vrije_invoer_image'] ) ? $theme_options['theme_xx2_vrije_invoer_image'] : null;
$theme_xx2_vrije_invoer_url   = isset( $theme_options['theme_xx2_vrije_invoer_url'] ) ? $theme_options['theme_xx2_vrije_invoer_url'] : '';
$theme_xx2_vrije_invoer_label = isset( $theme_options['theme_xx2_vrije_invoer_label'] ) ? $theme_options['theme_xx2_vrije_invoer_label'] : '';
...
function get_vrije_invoer( $theme_options = array() ) {
	...
	if ( $theme_xx2_vrije_invoer_title && $theme_xx2_vrije_invoer_text ) {
		...
		$imageURL_start     = '';
		$imageURL_end       = '';
		$image_alt          = $theme_xx2_vrije_invoer_title;
		$titel              = '<strong><span style="color:#000; font-size:24px; line-height:32px;">' . $theme_xx2_vrije_invoer_title . '</span></strong>';
		$vrije_invoer_label = '<p class="null"><span style="font-size:14px"><span
                                            style="color: #696969;font-weight: 600;">' . $theme_xx2_vrije_invoer_label . '</span></span>
                            </p>';

		// Do we have a valid URL?
		if ( filter_var( $theme_xx2_vrije_invoer_url, FILTER_VALIDATE_URL ) === false ) {
			// not a valid URL, keep the title as is
		} else {
			// append a link to the title

			$titel = '<a href="' . $theme_xx2_vrije_invoer_url . $theme_piwiktrackercode . '"><strong><span style="color:#01689B; font-size:24px; line-height:32px;">' . $theme_xx2_vrije_invoer_title . '</span></strong></a>';

			// prepare the link to the image
			$imageURL_start = '<a href = "' . $theme_xx2_vrije_invoer_url . $theme_piwiktrackercode . '" role = "presentation" tabindex = "-1">';
			$imageURL_end   = '</a>';

		}


		if ( is_array( $theme_xx2_vrije_invoer_image ) ) {

			$image = wp_get_attachment_image_src( $theme_xx2_vrije_invoer_image['id'], $entry_image_size );

			if ( $image ) {

				if ( ! $imageURL_start ) {
					// no link for this image; do not use the title as alt text, but get a different alt text
					$attachment = get_post( $theme_xx2_vrije_invoer_image['id'] );
					$image_alt  = get_post_meta( $theme_xx2_vrije_invoer_image['id'], '_wp_attachment_image_alt', true );
					if ( ! $image_alt ) {
                ...
                <table role="presentation" align="left" border="0"
                    ...
                    <tr>
                        <td class="mcnTextContent" valign="top"
                            ...
                            <p style="color:#000; font-size: 18px">' . $theme_xx2_vrije_invoer_text . '</p>
                        </td>
                    </tr>
                    </tbody>
                </table>
